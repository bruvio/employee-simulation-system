...
def test_advanced_story_export():
    ...
    try:
        ...
        print(f"   Export formats: json, csv, excel, xml, markdown")
        print("")
        ...
        print(f"   Stories: {sum(len(stories) for stories in sample_stories.values())} across {len(sample_stories)} categories")
        ...
        all_comparative_passed = all(test['file_exists'] and test['file_size'] > 50 for test in comparative_tests.values())
        ...
        capability_tests = {
            ...
            'supported_formats_documented': 'export_capabilities' in export_summary and 'supported_formats' in export_summary['export_capabilities'],
            'features_documented': 'export_capabilities' in export_summary and 'features' in export_summary['export_capabilities'],
        ...
        if 'markdown' in export_files:
            ...
            try:
                ...
                content_tests['markdown_has_categories'] = any(cat.replace('_', ' ').title() in md_content for cat in sample_stories.keys())
            except Exception:
        ...
        passed_checks = (
            sum(1 for test in export_tests.values() if test['file_exists'] and test['file_size'] > 100) +
            sum(1 for test in comparative_tests.values() if test['file_exists'] and test['file_size'] > 50) +
        ...
        print(f"   Total file size: {sum(test['file_size'] for test in export_tests.values()) + sum(test['file_size'] for test in comparative_tests.values()):,} bytes")
