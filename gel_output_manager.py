...
class GELOutputManager:
    ...
    def organize_gel_outputs(
        ...
        if chart_files:
            final_charts = self._organize_asset_files(
                chart_files, run_directories["charts"], "charts", cleanup_temp
            )
            final_paths["charts"] = final_charts
        
        if table_files:
            final_tables = self._organize_asset_files(
                table_files, run_directories["tables"], "tables", cleanup_temp
            )
    ...
    def validate_gel_output(self, run_directory: Path) -> Dict[str, Any]:
        ...
        if manifest_path.exists():
            try:
                ...
                required_keys = [
                    "scenario", "org", "timestamp_utc", "population", 
                    "median_salary", "below_median_pct", "gender_gap_pct"
                ]
...
if __name__ == "__main__":
    ...
    final_paths = manager.organize_gel_outputs(
