...
class TestEmployeePopulationGenerator:
    ...
    def test_generate_population_level_distribution(self):
        ...
        expected_dist = self.config["level_distribution"]
    ...
    def test_generate_population_salary_constraints(self):
        ...
        salary_constraints = self.config["salary_constraints"]
        ...
        for employee in population:
            ...
            if level in salary_constraints:
                ...
                assert adjusted_min <= salary <= adjusted_max, \
    ...
    def test_generate_population_tenure_years_valid(self):
        """Test that tenure years are valid."""
        population = self.generator.generate_population()
        
        for employee in population:
            assert 0 <= employee["tenure_years"] <= 40, \
                f"Invalid tenure: {employee['tenure_years']}"

    def test_generate_population_departments_assigned(self):
        """Test that all employees are assigned to departments."""
        population = self.generator.generate_population()
        
        valid_departments = ["Engineering", "Marketing", "Sales", "HR", "Finance", "Operations"]
        
        for employee in population:
            assert employee["department"] in valid_departments
    ...
    def test_generate_population_different_with_different_seed(self):
        ...
        differences = 0
        for i in range(len(pop1)):
            if (pop1[i]["level"] != pop2[i]["level"] or 
                pop1[i]["salary"] != pop2[i]["salary"] or
                pop1[i]["performance_rating"] != pop2[i]["performance_rating"]):
                differences += 1
    ...
    def test_generate_population_small_size(self):
        ...
        for employee in population:
            ...
            assert employee["performance_rating"] in [
                "Not met", "Partially met", "Achieving", "High Performing", "Exceeding"
            ]
