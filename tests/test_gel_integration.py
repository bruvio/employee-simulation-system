...
class TestGELIntegration:
    ...
    def test_complete_gel_workflow(self):
        ...
        # Generate reports
        md_report = md_builder.build_gel_report(
            self.sample_payload, 
            self.sample_manifest, 
            "test_report.md"
        )
    ...
    def test_gel_output_cleanup(self):
        ...
        for i in range(7):  # Create 7 runs
            ...
            run_dirs = output_manager.create_gel_run_directory(
                org="TestOrg",
                timestamp=timestamp,
                create_latest_link=False  # Don't create latest link for test
            )
            old_runs.append(run_dirs["run_root"])
        
        # Test dry run cleanup (keep 3)
        to_delete = output_manager.cleanup_old_runs(
            org="TestOrg", 
            keep_recent=3, 
            dry_run=True
        )
        ...
        # Test actual cleanup
        deleted = output_manager.cleanup_old_runs(
            org="TestOrg", 
            keep_recent=3, 
            dry_run=False
        )
    ...
    def test_report_content_consistency(self):
        ...
        key_info = [
            ...
            "201 employees" if "201" in str(self.sample_manifest["population"]) else str(self.sample_manifest["population"]),
            "£71,500" if "71500" in str(self.sample_manifest["median_salary"]) else str(self.sample_manifest["median_salary"]),
            "42.3%" if "42.3" in str(self.sample_manifest["below_median_pct"]) else str(self.sample_manifest["below_median_pct"]),
            "6.8%" if "6.8" in str(self.sample_manifest["gender_gap_pct"]) else str(self.sample_manifest["gender_gap_pct"])
        ]
    ...
    def test_concurrent_run_handling(self):
        ...
        run_dirs_1 = output_manager.create_gel_run_directory(
            org="TestOrg",
            timestamp=timestamp
        )
        
        run_dirs_2 = output_manager.create_gel_run_directory(
            org="TestOrg", 
            timestamp=timestamp
        )
...
class TestGELCLIIntegration:
    ...
    def test_cli_workflow_simulation(self, mock_loader, mock_output_manager):
        ...
        config = {
            "enable_gel_reporting": True,
            "gel_org": "GEL",
            "gel_roles_config_path": "config/orgs/GEL/roles.yaml"
        }
