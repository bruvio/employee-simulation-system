...
LEVEL_MAPPING = {1: "competent", 2: "advanced", 3: "expert", 4: "competent", 5: "advanced", 6: "expert"}
...
class EmployeePopulationGenerator:
    ...
    def __init__(
        ...
        self.salary_constraints = (
            salary_constraints if salary_constraints is not None else self.default_salary_constraints
        )
        ...
        LOGGER.info(
            f"Salary constraints: L1: £{self.salary_constraints[1]['min']:,}-£{self.salary_constraints[1]['max']:,}, L2: £{self.salary_constraints[2]['min']:,}-£{self.salary_constraints[2]['max']:,}, L3: £{self.salary_constraints[3]['min']:,}-£{self.salary_constraints[3]['max']:,}"
        )
    ...
    def generate_population(self):
        ...
        genders = self.rng.choice(["Male", "Female"], size=self.population_size, p=[0.65, 0.35])
    ...
    def _generate_constrained_salaries(self, levels):
        ...
        for level in [1, 2, 3, 4, 5, 6]:
            ...
            level_salaries = self._generate_median_constrained(
                target_median=median_target, size=level_count, min_salary=min_salary, max_salary=max_salary
            )
            ...
            LOGGER.debug(
                f"Level {level}: {level_count} employees, range £{level_salaries.min():.0f}-£{level_salaries.max():.0f}, median £{np.median(level_salaries):.0f}"
            )
    ...
    def _apply_negotiation_effects(self, salaries, level, count):
        ...
        constraints = self.salary_constraints[level]
        max_negotiated = constraints["max"] + (
            boost_amounts.max() if level == 3 else 0
        )  # Allow Level 3 to exceed normal max

        salaries[hard_negotiator_indices] += boost_amounts
        salaries[hard_negotiator_indices] = np.clip(
            salaries[hard_negotiator_indices], constraints["min"], max_negotiated
        )

        LOGGER.debug(
            f"Level {level}: {hard_negotiators_count} hard negotiators got salary boosts (avg £{boost_amounts.mean():.0f})"
        )
    ...
    def _generate_median_constrained(self, target_median, size, min_salary, max_salary):
        ...
        for iteration in range(max_iterations):
            ...
            if iteration == max_iterations - 1:
                LOGGER.warning(
                    f"Did not fully converge to target median. Final: £{current_median:.2f}, Target: £{target_median:.2f}"
                )

        final_median = np.median(salaries)
        LOGGER.debug(
            f"Final median: £{final_median:.2f} (target: £{target_median:.2f}, difference: £{abs(final_median - target_median):.2f})"
        )
    ...
    def _log_population_statistics(self, employees):
        ...
        for level in sorted(df["level"].unique()):
            ...
            category = "Senior" if level >= 4 else "Core"
            LOGGER.info(
                f"  Level {level} ({category}): median £{level_df['salary'].median():.2f}, "
...
def validate_salary_constraints(employees):
    ...
    if len(senior_employees) > 0:
        ...
        if difference <= tolerance:
            ...
            LOGGER.error(
                f"✗ Senior median salary constraint failed: £{senior_median:.2f} (target: £{target_median:.2f}, diff: £{difference:.2f})"
            )
            return False
    else:
