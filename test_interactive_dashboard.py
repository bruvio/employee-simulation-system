...
def test_interactive_dashboard():
    ...
    try:
        ...
        phase4_checks = {
            ...
            'story_tracker_available': hasattr(orchestrator, 'story_tracker') and orchestrator.story_tracker is not None
        }
        ...
        total_checks = sum(len(list(group)) if hasattr(group, '__len__') else 
                          sum(len(list(comp.values())) for comp in group) if all(isinstance(comp, dict) for comp in group) else
                          len(list(group)) 
