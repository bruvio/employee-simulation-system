...
class VisualizationGenerator:
    ...
    def __init__(self, population_data=None, inequality_progression=None, story_tracker=None):
        ...
        if self.story_tracker:
            ...
            LOGGER.info(f"Story tracking enabled with {sum(len(employees) for employees in self.tracked_employees.values())} tracked employees")
        
        self.setup_plotting_style()
        
        LOGGER.info(f"Initialized VisualizationGenerator with {len(population_data) if population_data else 0} employees")
    ...
    def generate_complete_analysis(self):
        ...
        if self.story_tracker and self.tracked_employees:
            ...
            visualizations['story_salary_distributions'] = self.save_figure(story_salary_fig, "story_salary_distributions")
    ...
    def plot_population_overview(self):
        ...
        bars1 = axes[0,0].bar(level_counts.index, level_counts.values, 
        ...
        axes[1,0].axvline(df['salary'].median(), color='red', linestyle='--', linewidth=2, 
                         label=f'Median: £{df["salary"].median():,.0f}')
        axes[1,0].axvline(df['salary'].mean(), color='orange', linestyle='--', linewidth=2,
    ...
    def plot_gender_analysis(self):
        ...
        axes[0,0].text(0.5, 0.95, f'Pay Gap: {gap_pct:.1f}%', transform=axes[0,0].transAxes, 
                      ha='center', fontweight='bold', bbox=dict(boxstyle='round', facecolor='wheat'))
        ...
        if 'Male' in level_gender_medians.columns and 'Female' in level_gender_medians.columns:
            ...
            axes[1,0].bar(x - width/2, male_medians, width, label='Male', 
                         color=self.colors['male'], alpha=0.7)
            axes[1,0].bar(x + width/2, female_medians, width, label='Female', 
                         color=self.colors['female'], alpha=0.7)
            ...
            if gap_percentages:
                bars = axes[1,1].bar(levels_with_both, gap_percentages, alpha=0.7, 
    ...
    def plot_performance_analysis(self):
        ...
        wedges, texts, autotexts = axes[0,0].pie(perf_counts.values, labels=perf_counts.index, 
                                                 autopct='%1.1f%%', colors=colors_perf, startangle=90)
        axes[0,0].set_title('Performance Rating Distribution')
        
        # Salary by performance violin plot
        perf_order = ['Not met', 'Partially met', 'Achieving', 'High Performing', 'Exceeding']
        perf_salary_data = [df[df['performance_rating'] == perf]['salary'].values 
                           for perf in perf_order if perf in df['performance_rating'].values]
        ...
        perf_mapping = {'Not met': 1, 'Partially met': 2, 'Achieving': 3, 
                       'High Performing': 4, 'Exceeding': 5}
        df['perf_numeric'] = df['performance_rating'].map(perf_mapping)
        
        scatter = axes[1,1].scatter(df['perf_numeric'], df['salary'], c=df['level'], 
        ...
        axes[1,1].set_xticklabels(['Not met', 'Partially\nmet', 'Achieving', 
    ...
    def plot_salary_distributions(self):
        ...
        axes[0,0].hist(df['salary'], bins=40, alpha=0.7, color=self.colors['primary'], 
        ...
        axes[0,1].hist(core_salaries, alpha=0.6, label=f'Core (L1-3): n={len(core_salaries)}', 
                      bins=25, color=self.colors['core'], density=True)
        axes[0,1].hist(senior_salaries, alpha=0.6, label=f'Senior (L4-6): n={len(senior_salaries)}', 
                      bins=25, color=self.colors['senior'], density=True)
        ...
        box_plot = axes[1,0].boxplot(level_salary_data, labels=[f'L{i}' for i in range(1, 7)], 
                                    patch_artist=True)
    ...
    def plot_inequality_reduction(self):
        ...
        axes[0,0].plot(cycles, df['gini_coefficient'], 'o-', linewidth=2, 
                      markersize=8, color=self.colors['primary'], label='Gini Coefficient')
        ...
        p = np.poly1d(z)
        axes[0,0].plot(cycles, p(cycles), "--", alpha=0.7, color='red', 
                      label=f'Trend: {"↗" if z[0] > 0 else "↘"} {abs(z[0]):.6f}/cycle')
        axes[0,0].legend()
        
        # Gender pay gap over time
        axes[0,1].plot(cycles, df['gender_gap_percent'].abs(), 's-', linewidth=2, 
                      markersize=8, color=self.colors['secondary'], label='Gender Pay Gap')
        ...
        axes[1,0].plot(cycles, df['median_salary'], '^-', linewidth=2, 
                      markersize=8, color='green', label='Median Salary')
        ...
        axes[1,1].plot(cycles, df['performance_salary_correlation'], 'd-', linewidth=2, 
                      markersize=8, color='purple', label='Performance-Salary Correlation')
    ...
    def plot_story_salary_distributions(self):
        ...
        for i, level in enumerate(levels):
            ...
            ax.hist(level_data['salary'], bins=15, alpha=0.6, color=self.colors['primary'], 
                   edgecolor='black', label='All employees')
            ...
            for category, employee_ids in self.tracked_employees.items():
                ...
                if tracked_at_level:
                    ax.scatter(tracked_at_level, [0.5] * len(tracked_at_level), 
                             color=self.story_colors[category], s=100, alpha=0.8, 
                             marker='o', label=f'{category.replace("_", " ").title()} ({len(tracked_at_level)})',
                             zorder=5)
    ...
    def plot_employee_progression_timelines(self):
        ...
        for i, (category, employee_ids) in enumerate(self.tracked_employees.items()):
            ...
            if category_data.empty:
                ax.text(0.5, 0.5, f'No data for {category.replace("_", " ").title()}', 
            ...
            for emp_id in employee_ids:
                ...
                if not emp_data.empty:
                    ...
                    ax.plot(emp_data_sorted['cycle'], emp_data_sorted['salary'], 
            ...
            ax.plot(avg_by_cycle.index, avg_by_cycle.values, 
                   color='black', linewidth=3, linestyle='--', 
                   label='Category Average', alpha=0.8)
    ...
    def create_interactive_story_dashboard(self):
        ...
        try:
            ...
            fig = px.line(timeline_df, x='cycle', y='salary', color='category', 
    ...
    def create_interactive_dashboard(self):
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Salary by Level', 'Gender Distribution', 
                           'Performance Analysis', 'Salary Distribution'),
        ...
        for level in sorted(df['level'].unique()):
            ...
            fig.add_trace(
                go.Box(y=level_data, name=f'Level {level}', boxpoints='outliers'),
                row=1, col=1
            )
        ...
        fig.add_trace(
            go.Scatter(x=df['perf_numeric'], y=df['salary'], mode='markers',
                      text=df['performance_rating'], hovertemplate='%{text}<br>Salary: £%{y:,.0f}',
                      marker=dict(color=df['level'], colorscale='viridis', 
                                showscale=True, colorbar=dict(title="Level")),
        ...
        # Salary histogram
        fig.add_trace(
            go.Histogram(x=df['salary'], nbinsx=30, name="Salary Distribution"),
            row=2, col=2
        )
        
        # Update layout
        fig.update_layout(
            title_text="Employee Population Interactive Dashboard",
            showlegend=False,
            height=800
        )
...
def create_parser():
    ...
    parser.add_argument("--output-dir", default="/Users/brunoviola/bruvio-tools/images/",
