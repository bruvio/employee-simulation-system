...
class InteractiveSalaryCalculator:
    ...
    def update_info_section(self, change=None):
        ...
        with self.info_output:
            ...
            print(f"  {level_tier.capitalize()}:      {uplift_data[level_tier]:.4f} ({uplift_data[level_tier]*100:.2f}%)")
            print()
    ...
    def display(self):
        ...
        results_section = widgets.VBox([
            widgets.HTML(value="<h3>📊 Calculation Results</h3>"),
            self.output
        ])
        
        # Create information section
        info_section = widgets.VBox([
            widgets.HTML(value="<h3>ℹ️ Uplift Matrix Details</h3>"),
            self.info_output
        ])
        
        # Create tabs for organized display
        tab_contents = [
            widgets.VBox([input_section, results_section]),
            info_section,
    ...
    def create_comparison_tool(self):
        ...
        def update_comparison(change=None):
            with comparison_output:
                ...
                a_total_uplift = (a_uplift_data['baseline'] + a_uplift_data['performance'] + a_uplift_data[a_level_tier]) * 100
                ...
                b_total_uplift = (b_uplift_data['baseline'] + b_uplift_data['performance'] + b_uplift_data[b_level_tier]) * 100
                ...
                print("-" * 65)
                print(f"{'Base Salary':<20} £{scenario_a_salary.value:<14,.2f} £{scenario_b_salary.value:<14,.2f} £{scenario_b_salary.value - scenario_a_salary.value:<14,.2f}")
                print(f"{'Performance':<20} {scenario_a_perf.value:<15} {scenario_b_perf.value:<15} {'-':<15}")
                print(f"{'Level':<20} {scenario_a_level.value:<15} {scenario_b_level.value:<15} {scenario_b_level.value - scenario_a_level.value:<15}")
                print(f"{'Total Uplift':<20} {a_total_uplift:<14.2f}% {b_total_uplift:<14.2f}% {b_total_uplift - a_total_uplift:<14.2f}%")
                print(f"{'New Salary':<20} £{a_new_salary:<14,.2f} £{b_new_salary:<14,.2f} £{b_new_salary - a_new_salary:<14,.2f}")
                print(f"{'Increase':<20} £{a_new_salary - scenario_a_salary.value:<14,.2f} £{b_new_salary - scenario_b_salary.value:<14,.2f} £{(b_new_salary - scenario_b_salary.value) - (a_new_salary - scenario_a_salary.value):<14,.2f}")
        ...
        # Set up observers
        widgets_to_observe = [scenario_a_salary, scenario_a_perf, scenario_a_level,
                             scenario_b_salary, scenario_b_perf, scenario_b_level]
