...
class EmployeeStoryTracker:
    ...
    def _identify_key_events(self, history: List[Dict]) -> List[str]:
        ...
        for i in range(1, len(history)):
            ...
            if curr['performance_rating'] != prev['performance_rating']:
                events.append(f"Cycle {curr['cycle']}: Performance changed from '{prev['performance_rating']}' to '{curr['performance_rating']}'")
    ...
    def _generate_story_summary(self, employee_id: int, category: str, history: List[Dict], 
                               total_growth_percent: float) -> str:
        """Generate narrative story summary based on category and history"""
        current_data = history[-1]
        initial_data = history[0]
        
        if category == 'gender_gap_affected':
            return (f"Employee {employee_id} is a female employee who started at Level {initial_data['level']} "
            ...
            return (f"Employee {employee_id} at Level {current_data['level']} has a salary of "
            ...
            return (f"Employee {employee_id} is a high performer at Level {current_data['level']} "
            ...
            return (f"Employee {employee_id} has been tracked for {len(history)} review cycles, "
                   f"with salary growth of {total_growth_percent:+.1f}% from £{initial_data['salary']:,.0f} "
                   f"to £{current_data['salary']:,.0f}.")
    
    def _generate_recommendations(self, employee_id: int, category: str, history: List[Dict], 
                                current_data: Dict) -> List[str]:
    ...
    def get_tracked_employee_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'total_population': sum(stats['total_employees'] for stats in self.population_stats.values()) // len(self.population_stats) if self.population_stats else 0
        }
