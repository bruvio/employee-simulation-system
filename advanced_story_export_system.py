...
class AdvancedStoryExportSystem:
    ...
    def export_employee_stories_comprehensive(self, 
        ...
        comprehensive_data = self._prepare_comprehensive_story_data(
            employee_stories, population_data, cycle_data
        )
    ...
    def _prepare_comprehensive_story_data(self, 
                                        employee_stories: Dict[str, List],
                                        population_data: List[Dict],
                                        cycle_data: Optional[pd.DataFrame] = None) -> List[Dict]:
            ...
            for story in stories:
                ...
                if cycle_data is not None and not cycle_data.empty:
                    ...
                    if not emp_cycles.empty:
                        ...
                        emp_cycles['performance_rating'] = pd.to_numeric(emp_cycles['performance_rating'], errors='coerce')
                        ...
                        story_data.update({
                            ...
                            'performance_improvement': float(emp_cycles['performance_rating'].iloc[-1] - emp_cycles['performance_rating'].iloc[0]) if len(emp_cycles) > 1 else 0
                        })
    ...
    def _export_stories_json(self, comprehensive_data: List[Dict], 
                           employee_stories: Dict[str, List]) -> Path:
        ...
        for category in employee_stories.keys():
            ...
            if category_stories:
                ...
                growths = [story.get('total_growth_percent', 0) for story in category_stories if story.get('total_growth_percent')]
    ...
    def _export_stories_excel(self, comprehensive_data: List[Dict], 
                            employee_stories: Dict[str, List]) -> Path:
    ...
    def _export_stories_xml(self, comprehensive_data: List[Dict], 
                          employee_stories: Dict[str, List]) -> Path:
    ...
    def _export_stories_markdown(self, comprehensive_data: List[Dict], 
                               employee_stories: Dict[str, List]) -> Path:
        ...
        if comprehensive_data:
            ...
            lines.append(f"- **Positive Growth:** {positive_growth} employees ({positive_growth/len(growths)*100:.1f}%)")
            lines.append(f"- **Negative Growth:** {negative_growth} employees ({negative_growth/len(growths)*100:.1f}%)")
            lines.append(f"- **Average Growth:** {sum(growths)/len(growths):.1f}%")
            lines.append("")
