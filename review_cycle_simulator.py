...
class ReviewCycleSimulator:
    ...
    def __init__(self, initial_population, random_seed=42):
        ...
        LOGGER.info(f"Initial inequality metrics - Gini: {initial_metrics['gini_coefficient']:.4f}, "
    ...
    def _calculate_inequality_metrics(self, cycle):
        ...
        if len(male_salaries) > 0 and len(female_salaries) > 0:
            ...
            for level in range(1, 7):
                level_males = [emp['salary'] for emp in self.population 
                              if emp['gender'] == 'Male' and emp['level'] == level]
                level_females = [emp['salary'] for emp in self.population 
                                if emp['gender'] == 'Female' and emp['level'] == level]
                
                if len(level_males) > 0 and len(level_females) > 0:
                    level_gap = ((np.median(level_males) - np.median(level_females)) / 
                               np.median(level_males) * 100)
        ...
        try:
            ...
            performance_correlation = float(corr_matrix[0, 1]) if corr_matrix.shape == (2, 2) and len(perf_numeric) > 1 else 0.0
        except:
    ...
    def _check_inequality_convergence(self, inequality_progression, current_cycle, lookback=3):
        ...
        if gini_converged and gender_gap_converged:
            LOGGER.debug(f"Convergence detected - Gini improvement: {gini_improvement:.6f}, "
    ...
    def _analyze_inequality_reduction(self, inequality_progression):
        ...
        gini_reduction_pct = (gini_reduction / initial['gini_coefficient'] * 100) if initial['gini_coefficient'] > 0 else 0
        ...
        median_salary_increase_pct = (median_salary_increase / initial['median_salary'] * 100) if initial['median_salary'] > 0 else 0
        
        # Performance-salary correlation improvement
        perf_correlation_improvement = final['performance_salary_correlation'] - initial['performance_salary_correlation']
        
        LOGGER.info(f"Total improvement over {len(inequality_progression)-1} cycles:")
        LOGGER.info(f"  Gini coefficient: {initial['gini_coefficient']:.4f} → {final['gini_coefficient']:.4f} "
                   f"(reduction: {gini_reduction:.4f}, {gini_reduction_pct:.1f}%)")
        LOGGER.info(f"  Gender pay gap: {initial['gender_gap_percent']:.2f}% → {final['gender_gap_percent']:.2f}% "
                   f"(reduction: {gender_gap_reduction:.2f}%)")
        LOGGER.info(f"  Median salary: £{initial['median_salary']:.2f} → £{final['median_salary']:.2f} "
                   f"(increase: £{median_salary_increase:.2f}, {median_salary_increase_pct:.1f}%)")
        LOGGER.info(f"  Performance-salary correlation: {initial['performance_salary_correlation']:.3f} → "
        ...
        cycles_for_significant_reduction = self._find_cycles_for_threshold(
            inequality_progression, 'gini_coefficient', 
            initial['gini_coefficient'], significant_reduction_threshold
        )
        
        if cycles_for_significant_reduction:
            LOGGER.info(f"Significant inequality reduction (50%) achieved after {cycles_for_significant_reduction} cycles")
        else:
    ...
    def save_simulation_results(self, inequality_progression, filename_prefix="simulation_results"):
        ...
        population_filepath = f"/Users/brunoviola/bruvio-tools/artifacts/{filename_prefix}_final_population_{timestamp}.csv"
    ...
    def _get_final_analysis_summary(self, inequality_progression):
        ...
        return {
            ...
            'gini_reduction_percentage': ((initial['gini_coefficient'] - final['gini_coefficient']) / 
                                        initial['gini_coefficient'] * 100) if initial['gini_coefficient'] > 0 else 0,
            'initial_gender_gap': initial['gender_gap_percent'],
            'final_gender_gap': final['gender_gap_percent'],
            'gender_gap_reduction': abs(initial['gender_gap_percent']) - abs(final['gender_gap_percent']),
            'median_salary_increase': final['median_salary'] - initial['median_salary'],
            'performance_correlation_improvement': final['performance_salary_correlation'] - initial['performance_salary_correlation']
        }
...
def validate_inequality_calculations():
    ...
    if gini_unequal > gini_moderate > gini_equal:
        LOGGER.info(f"✓ Inequality detection validation passed: {gini_unequal:.4f} > {gini_moderate:.4f} > {gini_equal:.4f}")
    else:
        LOGGER.error(f"✗ Inequality detection validation failed: {gini_unequal:.4f}, {gini_moderate:.4f}, {gini_equal:.4f}")
        return False
...
def create_parser():
    ...
    parser.add_argument("--population-file", 
                       help="JSON file with initial population data (required for simulation)")
    parser.add_argument("--consistency", type=float, default=0.7, 
                       help="Performance consistency rate 0.0-1.0 (default: 0.7)")
    parser.add_argument("--seed", type=int, default=42, help="Random seed (default: 42)")
    parser.add_argument("--validate-calculations", action="store_true",
                       help="Validate inequality calculation methods")
    parser.add_argument("--output-prefix", default="simulation_results",
                       help="Output file prefix (default: simulation_results)")
...
def main():
    ...
    inequality_progression = simulator.simulate_multiple_cycles(
        ...
        performance_consistency=args.consistency
    )
