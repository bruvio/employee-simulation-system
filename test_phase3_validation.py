...
def test_phase3_validation():
    ...
    try:
        ...
        phase3_checks = {
            ...
            'progress_indicators_enabled': orchestrator.smart_logger.enable_progress if hasattr(orchestrator, 'smart_logger') else False
        }
        ...
        expected_dirs = [
            'run_root', 'population_data', 'simulation_results', 
            'reports', 'logs', 'employee_stories', 'story_analysis'
        ]
        ...
        total_checks = len(phase3_checks) + len(directory_checks) + len(metadata_checks) + len(summary_checks) + len(org_checks)
        passed_checks = sum([
