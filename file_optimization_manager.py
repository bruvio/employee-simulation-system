...
class FileOptimizationManager:
    ...
    def organize_population_files(self, population_data: List[Dict], 
                                cycle_progressions: Optional[pd.DataFrame] = None) -> Dict[str, str]:
        """
    ...
    def organize_simulation_results(self, inequality_metrics: pd.DataFrame, 
                                  convergence_info: Dict[str, Any]) -> Dict[str, str]:
        """
        ...
        summary = {
            ...
            'final_gini_coefficient': float(inequality_metrics.iloc[-1]['gini_coefficient']) if not inequality_metrics.empty else None,
    ...
    def organize_story_files(self, employee_stories: Dict[str, List], 
                           story_timeline: Optional[pd.DataFrame] = None) -> Dict[str, str]:
        """
        ...
        if not self.run_structure or 'employee_stories' not in self.run_structure:
            raise ValueError("Story tracking directories not available. Enable story tracking when creating run directory.")
