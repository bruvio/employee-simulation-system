...
class InterventionPolicy(BaseModel):
    ...
    inequality_budget_percent: float = Field(
        ...
        ge=0.0, 
        le=5.0,
        description="Budget percentage (0.5 = 0.5%) for inequality remediation"
    )
...
class Role(BaseModel):
    ...
    min_salaries: List[float] = Field(
        description="List of minimum salaries (supports multiple bands)",
        min_items=1
    )
    headcount_hint: Optional[int] = Field(
        default=None,
        ge=0,
        description="Optional expected headcount for planning"
    )
...
class RolesConfig(BaseModel):
    ...
    intervention_policy: InterventionPolicy = Field(
        ...
        description="Manager intervention constraints"
    )
...
class RolesConfigLoader:
    ...
    def validate_config_file(self, config_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            roles_with_multiple_bands = sum(
                1 for role in config.roles if len(role.min_salaries) > 1
            )
