...
class DataExportSystem:
    ...
    def _export_json_simulation(self, simulation_data, filepath):
        ...
        if isinstance(simulation_data, dict):
            ...
            export_data = {
                'simulation_results': simulation_data.to_dict('records')
            }
    ...
    def _export_json_analysis(self, population_data, simulation_results, analysis_summary, filepath):
        ...
        export_data = {
            ...
            'population_data': population_data if isinstance(population_data, list) else population_data.to_dict('records'),
...
def main():
    ...
    parser.add_argument('--formats', nargs='+', choices=['csv', 'excel', 'json'], 
    ...
    if args.analysis_report and simulation_data is not None:
        ...
        exported_files = exporter.export_analysis_report(
            population_data, simulation_data, format_types=args.formats
        )
    else:
        # Export individual datasets
        exported_files = exporter.export_employee_population(
            population_data, format_types=args.formats
        )
        
        if simulation_data is not None:
            sim_files = exporter.export_simulation_results(
                simulation_data, format_types=args.formats
            )
