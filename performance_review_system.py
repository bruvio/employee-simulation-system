...
class PerformanceReviewSystem:
    ...
    def assign_performance_ratings(self, employees):
        ...
        performance_counts = {'Not met': 0, 'Partially met': 0, 'Achieving': 0, 
                            'High Performing': 0, 'Exceeding': 0}
    ...
    def validate_uplift_calculations(self, test_cases=None):
        ...
        for test_case in test_cases:
            ...
            if not test_passed:
                ...
                LOGGER.error(f"  Expected uplift: {expected_total:.2f}%, Got: {uplift_result['uplift_percentage']:.2f}%")
...
def create_parser():
    ...
    parser.add_argument("--test-uplift-calculation", action="store_true", 
                       help="Test uplift calculation accuracy")
    parser.add_argument("--apply-review", help="Apply review to population file (JSON format)")
    parser.add_argument("--review-year", type=int, default=1, 
                       help="Review year number (default: 1)")
...
def main():
    ...
    if args.test_uplift_calculation:
        ...
        # Save results
        csv_path, json_path = review_system.save_review_results(
            review_results, f"review_year_{args.review_year}"
        )
